...
parser.add_argument('--num_colors', default=1, type=int, help='')  # default=1  11 8
...
def create_avg_list(orig_list, skip_size=1, num_to_avg=1):
    for_avg_list = np.array(orig_list)
    avg_shape = list(np.array(orig_list).shape)
    avg_shape[2] = list(np.array(orig_list).shape)[2] // skip_size  # 2

    avg_list = np.zeros(avg_shape)
    for idx in range(0, np.array(orig_list).shape[2], skip_size):  # 2):
...
if __name__ == '__main__':
    ...
    if args.focused_ind in range(num_classes):
        ...
        if args.num_colors > 1 and num_conds > 1:
            ...
            avg_list = create_avg_list(for_graphs_list, 2, 2)
