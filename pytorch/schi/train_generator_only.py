...
parser.add_argument('--data_dir', default='data/grayscale', help="Directory containing the dataset")
...
def train(g_model, g_optimizer, mse_loss_fn, dataloader, params, epoch, fig):
    ...
    # Save Losses for plotting later
    D_losses.append(stats_mean['g_error'].item())
...
#                     for item in elem:
#                         net_grads_graph.append([min(item), np.median(item), max(item)])
#                 elif isinstance(elem, (list,)):
#                     net_grads_graph.append([min(elem), np.median(elem), max(elem)])
#                 else:
#                     net_grads_graph.append([elem])
#
