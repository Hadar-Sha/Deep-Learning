...
def get_stats_for_log(d_error, g_error, d_pred_real, d_pred_fake):
...
def train(d_model, g_model, d_optimizer, g_optimizer, loss_fn, dataloader, params, epoch, fig):
    ...
    for i, (real_batch, real_label) in enumerate(dataloader):
        ...
        stats = get_stats_for_log(d_error, g_error, d_pred_real, d_pred_fake)
    ...
    if (epoch + 1) % (0.01 * params.num_epochs) == 0:
        ...
        # Display Images
        display_results.fill_figure(test_samples_reshaped, fig, gan_net.labels_to_titles(test_labels))  # , epoch+1)
